...
- `--include-dev-as-real`：跳过 dev replay 启发式（同篇真实文章被多次鉴定的场景，避免被误标）
- `--token-outlier=N` / `--elapsed-outlier=N`：自定义 outlier 阈值（默认 200K / 120s）

## 真实 case 不足时的处理

跑完数据准备后，如果发现 **真实 case 数 < 5**，先不要急着写报告。检查：

1. **时间窗可能太短**：默认 `--last 7` 只看最近 7 天。先告诉用户"真实 case 只有 N 个，要不要扩大到 30 天/90 天看看？"，等用户回答后用 `--last 30` 或 `--last 90` 重跑
2. **可能 dev replay 误标**：如果用户最近反复鉴定同一真实文章（不是开发期 replay 脚本），dev 启发式会把这些当 dev。建议跑 `--include-dev-as-real` 让用户判断
3. **真没有就如实说**：n=2 时不要装作能给出趋势判断，明确说"数据不足，列举式呈现，不做统计推断"

脚本已经在 case<5 时自动改为"列举式"输出（不算 median/p95），但你**还要在报告里明确说明 n 太小、结论无统计意义**。

## 何时不应跑 /analyze-logs

跑前先看 `reports/` 下最近的 `log_analysis_*.md`：
- 如果**上次报告在 24h 内**写过、**真实 case 数没增加**（看上次报告的元数据），**告诉用户结论 80% 会一样，建议跳过**——避免空转
- 如果用户坚持跑（如想验证工具改动效果），按正常流程继续
- 真正值得重跑的场景：用户跑了新真实 case、有新调优 commit、跨过 7+ 天
