...
**搜索前必做：声明提取（Planning Step）**
在第一次搜索前，先从截图内容中提取 1-4 条核心可验证声明（一句话概括，去掉主观评论），然后**针对每条声明独立搜索**，而非泛搜整张截图内容。每条声明至少尝试 2-3 个不同关键词组合。
...
## ⚠️ 传播链分析规则（当截图包含多人转发/评论时必须执行）

当截图内容是多人的社交媒体转发链（A发帖→B评论→C分析→D引用），必须完成以下判断：

1. **识别原始信源**：谁是最初的声明者？原始信源的 source_type 决定整个传播链的信源上限
2. **技术分析 ≠ 独立核实**：后续用户对截图/文件进行技术解读（"这个软件有漏洞"、"这台机器在做X计算"）属于**条件性推断**（若原始声明为真则推论成立），不构成对原始事件的独立核实，不提升 effective_sources 计数
3. **分析者的专业性 ≠ 事件的可信度**：即使分析者技术分析准确，其分析的前提（未核实的截图/文件）的真实性是独立问题
...
6. **⚠️ 核实一致性强制规则**：检查 claim_verification 里的所有 verdict：
   - 若**全部 verdict 均为「✓ 独立核实属实」或「✓ 官方自述」，零条「✗ 伪造」**，则 bullshit_index **必须 ≤ 30**，risk_level **必须为「✅ 基本可信」**。不得用 50 表示"不确定"——50 的字面含义是"一半内容是假的"，而非"我有点不确定"。
   - 若有部分声明「? 无法核实」但无伪造，bullshit_index 上限为 45。
   - 只有存在「✗ 伪造」的声明时，bullshit_index 才允许超过 50。
...
{{
  ...
  "investigation_report": {{
    ...
    "physics_check": "物理/常识/视觉篡改核查：① 内容是否违背基础科学定律或基本常识；② 若图中包含视频截图或照片，注意检查是否存在左右镜像翻转（特征：水印/文字/Logo呈镜像状态、人物惯用手异常、场景布局与常识相反）——视频被翻转通常是规避平台重复内容检测的手段，属于内容操纵信号，应在 flaw_list 中明确指出",
...
### Step 1：视觉初判
- 分析画面风格（画风、色调、角色造型、场景设计）
- 识别可能的角色特征（发型、服装、肤色、标志性道具）
- 读取图中所有可见文字（对话、旁白、杂志名、集数标注等）
- **检查图中是否存在镜像翻转**：若水印/Logo/文字呈镜像状态，说明图片被左右翻转，识别时应还原后再读取
- **判断内容来源类别**：
  - 若图中存在平台 UI 元素（点赞/转发数、头像、@符号、帖子界面）→ 这是社交媒体截图，其中的视频/图片才是待识别对象
  - 若画面特征明显是 AI 生成（过度完美、肢体失真、不合理构图、AI 水印）→ 记录为 AI 生成内容，found=false
  - 仅当确认是来自真实动漫/游戏/影视作品的截图时，才进行作品识别
- 形成初步判断：作品名（或候选列表）、媒体类型

### Step 2：工具调用（强制执行）
**可在同一轮同时发起 reverse_image_search 和 web_search**（两者并行执行，速度更快）。

- **reverse_image_search**（若可用）：以图搜图结果最精准，优先调用，直接给出作品名和来源页面
- **web_search**：搜索图中可见文字、角色特征、杂志名，英文搜索扩大覆盖，如"[character description] manga which series"

确认作品原名后，**必须单独再搜索其中文译名**（这是必须执行的步骤，不可跳过）：
- 搜索格式："[原作名] 拷贝漫画" 或 "[原作名] 漫画柜" 或 "[原作名] 动漫之家"
- 以中文平台上实际显示的标题为准，**禁止根据原名字面意思自行翻译**
- 注意：原名中的片假名词汇可能是复合缩写（如 バツイチ+ハーレム→バツハレ），其汉化译名往往与字面完全不同，必须搜索确认

### Step 3：综合判断
- 搜索到佐证：confidence=high 或 medium，将证据写入 note
- 搜索无佐证但视觉判断清晰：confidence=medium，note 注明"基于视觉判断，未找到搜索佐证"
- 完全无法识别：found=false

## 输出规则（重要）

**title（中文名）**：必须通过搜索确认汉化组/平台通行的中文译名，**禁止自行翻译原名**。原名中的词汇（尤其是片假名）可能有完全不同于字面的汉化译名，务必搜索后填写，不得猜测。若搜索无结果，填写原文名并在 note 中注明"未找到中文译名"。

**episode（集数/话数）**：
- 漫画页面常见的"END"标记表示**本话结束**，不代表连载完结
- 填写格式：连载中写"第X话（最新话）"，已确认完结写"第X话（最终话）"，不确定是否完结不加括号注释

**characters（角色名）**：
- **只填已通过搜索确认的角色名**，凭视觉猜测的一律留空数组 `[]`
- 搜索方式：在拷贝漫画/漫画柜/动漫之家等中文平台查找该作品的角色介绍页，以平台显示的中文名为准
- **禁止直译假名读音**，禁止根据日文名猜测中文名，不确定就留空
- 若角色名留空，在 note 中注明"角色名未搜索确认，已留空"
...
{{
  ...
  "episode": "",
  "episode_title": "",
  "scene": "图中实际内容描述（即使找不到作品来源，也要描述画面中有什么内容、来自什么平台或场景）",
  "characters": [],
  "confidence": "low",
  "note": "无法识别的原因，以及对图片内容性质的判断（如：社交媒体截图、AI生成内容、真实照片等）"
}}
...
def get_follow_up_prompt(mode: str) -> str:
    ...
    return (
        ...
        "5. 你有 web_search 工具可以使用。以下情况必须先搜索再回答：\n"
        "   - 用户提到你不确定的缩写、圈内梗、产品型号、人名、事件（哪怕只是「这个词是什么意思」）\n"
        "   - 问题涉及近两年内的新闻、新产品、新梗\n"
        "   - 用户明确要求搜索\n"
        "   纯推理、解释已有背景、简单问答不必搜索\n"
        "6. 【诚实原则】搜索前不要猜测未知术语/缩写的含义，"
        "更不能把猜测当作事实陈述——先搜，搜不到再说不确定\n"
        "7. 【纠错原则】用户纠正你时，先完整接受纠正，再重新组织回答；"
