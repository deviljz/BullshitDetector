...
- **title_logic_check（标题逻辑核查）**：标题是否将相关性伪装成因果关系（A发生了+B发生了→暗示A导致B）；或用与文章主论点无关的数据/成就为标题结论背书（如用商业成功数字暗示某设计方案是成功的关键原因）；识别"标题党因果谬误"并单独指出
...
【示例1 — 游戏商业报道】
输入：首月销量破330万，两天回本，GDC分享获好评，事件卡组系统解决了开放世界最大痛点
输出：{{"claims": [
  {{"text": "首月销量突破330万份", "type": "fact"}},
  {{"text": "上线两天收回全部开发成本", "type": "fact"}},
  {{"text": "事件卡组系统是本作商业成功的核心原因，解决了开放世界设计的根本痛点", "type": "narrative"}}
]}}
...
输入：首月销量破330万，两天回本，解决了最大痛点
❌错误输出：{{"claims": [{{"text": "销量330万", "type": "fact"}}, {{"text": "两天回本", "type": "fact"}}]}}
原因：没有提取文章的核心叙事论点（"解决了最大痛点"）"""
...
- 同系列不同作品（如《对马岛之魂》≠《羊蹄山之魂》）的数据**不得**用于否定当前声明
- 同名但不同年份的演讲/发布会（如 GDC 2021 ≠ GDC 2026）**不得**视为矛盾
