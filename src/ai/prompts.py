...
【规则】
- 总计最多5条：fact 最多3条，narrative 至少1条最多2条
- **财务/商业数据必须提取为 fact**（销量/票房/收入/回本周期），这类最容易夸大
- **标题核心结论必须提取**（如"两天回本""330万份"）
- narrative 提取文章想让读者相信的整体结论，不提取纯主观褒贬
- 每条声明一句话，简洁
...
输出：{{"claims": [
  ...
  {{"text": "本作商业表现在系列中具有突破性，优于前作同期", "type": "narrative"}}
]}}"""
...
**步骤2 — 财务/商业数据类声明**（含"回本""利润""成本""销量""收入""票房"等）：
  ...
  - 违反行业常识（如 3A 游戏"两天回本"：开发成本通常数千万至数亿，平台分成后两天净收入不可能覆盖）且无详细财务核算来源 → "✗ 伪造"
  - 搜不到足够证据 → "? 无法核实"
