...
**⚠️ bullshit_nature 必填**：根据核查结论，从下列九项中选一个最贴切的标签填入 header.bullshit_nature，禁止留空、禁止填写描述性文字：
- 属实：全部声明均已核实（无任何 ✗ 或 ?），且无标题问题、无夸大、无叙事论点（**最高可信标签**）
...
**选标签决策树**：⓪ 全部 ✓、无 ?、无夸大、无标题问题、无叙事论点 → 属实；① 伪造数 ≥ 已核实数 → 事实错误；② 多处重要声明存疑但核心尚成立 → 局部失实；③ 绝大多数属实仅细节有误 → 基本属实；④ 全部属实但核心叙述被夸大 → 夸大渲染；⑤ 全部属实（claim_verification 无任何 ✗）且无夸大但内容荒诞 → 真实但离谱（**有 ✗ 时绝对禁止选此项**）；⑥ 标题误导 → 标题党；⑦ 语境剥离 → 断章取义；⑧ 推理链断裂 → 逻辑混乱。
