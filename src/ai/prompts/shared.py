...
- 真实但离谱：全部内容属实，但事情本身荒诞令人咋舌（全部 ✓，BI 低）
...
**选标签决策树**：① 伪造数 ≥ 已核实数 → 事实错误；② 多处重要声明存疑但核心尚成立 → 局部失实；③ 绝大多数属实仅细节有误 → 基本属实；④ 全部属实但核心叙述被夸大 → 夸大渲染；⑤ 全部属实且无夸大但内容荒诞 → 真实但离谱；⑥ 标题误导 → 标题党；⑦ 语境剥离 → 断章取义；⑧ 推理链断裂 → 逻辑混乱。
